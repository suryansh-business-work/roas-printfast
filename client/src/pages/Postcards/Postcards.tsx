...
const Postcards = () => {
  ...
  const [editTarget, setEditTarget] = useState<IPostcard | null>(null);
  const [confirmOpen, setConfirmOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<IPostcard | null>(null);
  ...
  const fetchPostcards = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        setPostcards(response.data.items); setTotalItems(response.data.totalItems);
      }
    } catch { /* silent */ } finally { setIsLoading(false); }
  }, [page, rowsPerPage, sortField, sortOrder, search, filters]);

  useEffect(() => { fetchPostcards(); }, [fetchPostcards]);
  ...
  const handleEdit = (row: IPostcard) => { setEditTarget(row); setEditDialogOpen(true); };
  const handleDelete = (row: IPostcard) => { setDeleteTarget(row); setConfirmOpen(true); };

  const handleConfirmDelete = async () => {
    if (!deleteTarget) return;
    setIsDeleting(true);
    try {
      await postcardService.bulkDeactivatePostcards([deleteTarget.postcardId]);
      showSnackbar('Postcard deactivated successfully', 'success');
      setConfirmOpen(false); setDeleteTarget(null); fetchPostcards();
    } catch { showSnackbar('Failed to deactivate postcard', 'error'); }
  ...
  const handleConfirmBulkDelete = async () => {
    setIsDeleting(true);
    try {
      await postcardService.bulkDeactivatePostcards(bulkDeleteIds);
      showSnackbar(`${bulkDeleteIds.length} postcard(s) deactivated`, 'success');
      setConfirmBulkOpen(false); setBulkDeleteIds([]); setSelectedRows([]); fetchPostcards();
    } catch { showSnackbar('Failed to deactivate postcards', 'error'); }
    finally { setIsDeleting(false); }
  };

  const handleEditSuccess = () => {
    setEditDialogOpen(false); setEditTarget(null);
    showSnackbar('Postcard updated successfully', 'success'); fetchPostcards();
  };
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => setDialogOpen(true)}>Add Postcard</Button>
      </Box>
      <DataTable columns={columns} rows={postcards} totalItems={totalItems} page={page} rowsPerPage={rowsPerPage}
        ...
        isLoading={isLoading} getRowKey={(row) => row.postcardId}
        selectable selectedRows={selectedRows} onSelectionChange={setSelectedRows} onBulkDelete={handleBulkDelete}
      />
      <CreatePostcardDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSuccess={fetchPostcards} />
      <EditPostcardDialog open={editDialogOpen} postcard={editTarget}
        onClose={() => { setEditDialogOpen(false); setEditTarget(null); }} onSuccess={handleEditSuccess} />
      <ConfirmDialog open={confirmOpen} title="Deactivate Postcard"
      ...
      <ConfirmDialog open={confirmBulkOpen} title="Deactivate Postcards"
        message={`Are you sure you want to deactivate ${bulkDeleteIds.length} postcard(s)?`}
