...
interface CreatePostcardDialogProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}
...
const CreatePostcardDialog = ({ open, onClose, onSuccess }: CreatePostcardDialogProps) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        const response = await postcardService.createPostcard({
          name: values.name,
        ...
        setError(error.response?.data?.error?.message || 'Failed to create postcard');
      }
    },
  });
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <DialogTitle>Create Postcard</DialogTitle>
...
export default CreatePostcardDialog;
